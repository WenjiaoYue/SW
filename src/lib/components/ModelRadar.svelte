<script lang="ts">
  ...
  import { Cpu, BarChart3, Database, AlertCircle, XCircle, CheckCircle } from 'lucide-svelte';
  ...
  $: stats.xpuUnsupported = stats.total - stats.xpuSupported;
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-5 text-white shadow-lg">
        <div class="flex items-center justify-between mb-3">
          ...
          <span class="text-2xl font-bold">{stats.xpuUnsupported}</span>
        </div>
        <div class="text-red-100 text-sm font-medium">XPU Unsupported</div>
        <div class="mt-2 flex items-center gap-2">
          <div class="flex-1 bg-white/20 rounded-full h-1.5">
            <div class="bg-white rounded-full h-1.5 transition-all" style="width: {stats.total > 0 ? (stats.xpuUnsupported / stats.total * 100) : 0}%"></div>
          </div>
          <span class="text-xs font-semibold">{stats.total > 0 ? Math.round(stats.xpuUnsupported / stats.total * 100) : 0}%</span>
        </div>
      </div>
    </div>
