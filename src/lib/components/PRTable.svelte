<script lang="ts">
  ...
  function handleMinChange() {
    filters.minRelevanceScore = Math.min(
  ...
  function handleMaxChange() {
    filters.maxRelevanceScore = Math.max(
      filters.maxRelevanceScore,
      filters.minRelevanceScore,
    );
  }
  ...
  $: allFilteredPRs = (() => {
    let filtered = prs.filter((pr) => {
      if (
        ...
        !filters.hardware.includes(pr.hardware)
      ) {
        return false;
      }
      ...
      if (
        ...
        !pr.types.some((type) => filters.types.includes(type))
      ) {
        return false;
      }

      const score = pr.xpu_relevance_score;
        if (
          score < filters.minRelevanceScore ||
          score > filters.maxRelevanceScore
        ) {
          return false;
        }
    ...
    if (sortField) {
      filtered = [...filtered].sort((a, b) => {
        ...
        if (sortField === "xpu_relevance_score") {
          aValue = a.xpu_relevance_score;
          bValue = b.xpu_relevance_score;

          const aInvalid = aValue === -1;
          const bInvalid = bValue === -1;
  ...
  function toggleFilter(
    filterType: "hardware" | "state" | "types",
    value: string,
  ) {
...
<div
  class="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden"
>
  <div
    class="px-4 py-3 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between"
  >
    <div class="flex items-center gap-2">
      <GitPullRequest class="w-4 h-4 text-blue-600" />
      <h3 class="font-bold text-slate-800 text-sm">Pull Request Analysis</h3>
      <span class="text-xs text-slate-500"
        >({allFilteredPRs.length} of {prs.length})</span
      >
    </div>
    <div class="flex items-center gap-2">
      {#if hasActiveFilters}
        ...
        {#if hasActiveFilters}
          ...
          >
            {filters.hardware.length +
              ...
              (filters.minRelevanceScore > 0 || filters.maxRelevanceScore < 100
                ? 1
                : 0)}
          </span>
        {/if}
  ...
  {#if showFilters}
    <div class="px-4 py-3 bg-slate-50/50 border-b border-slate-200 space-y-3">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        <div>
          <label class="text-xs font-semibold text-slate-700 mb-1.5 block"
            >Hardware</label
          >
          <div class="flex flex-wrap gap-1.5">
            {#each uniqueHardware as hw}
              <button
                ...
                class="px-2 py-1 rounded text-[10px] font-medium border transition {filters.hardware.includes(
                  hw,
                )
                  ? getHardwareBadgeClass(hw)
          ...
          <div class="flex flex-wrap gap-1.5">
            {#each uniqueStates as st}
              <button
                ...
                class="px-2 py-1 rounded text-[10px] font-medium border transition {filters.state.includes(
                  st,
                )
                  ? getStateBadgeClass(st)
                  : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}"
              >
                {st}
              </button>
          ...
          <div class="flex flex-wrap gap-1.5">
            {#each uniqueTypes as type}
              <button
                ...
                class="px-2 py-1 rounded text-[10px] font-medium border transition {filters.types.includes(
                  type,
                )
        ...
        <div>
          <div
            class="flex justify-between items-center mb-1.5 whitespace-nowrap"
          >
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] text-slate-500 font-medium"
                >({allFilteredPRs.length} matches)</span
              >
            </div>
            <span
              class="text-[10px] text-slate-500 font-mono bg-slate-100 px-1.5 rounded whitespace-nowrap"
            >
              {filters.minRelevanceScore} - {filters.maxRelevanceScore}
            </span>
          </div>
          ...
          >
            <div
              class="absolute w-full h-1.5 bg-slate-200 rounded-full overflow-hidden"
            ></div>
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left">
      <thead
        class="bg-slate-50 text-[10px] uppercase text-slate-500 font-semibold border-b border-slate-200"
      >
        <tr>
          <th class="px-3 py-2.5 w-12">
            ...
            >
              ...
              {:else}
                <ArrowUpDown
                  class="w-3 h-3 opacity-0 group-hover:opacity-50 transition"
                />
              {/if}
            </button>
          </th>
            ...
            <tr class="hover:bg-slate-50/70 transition group">
              <td class="px-3 py-2.5">
                <div class="flex items-center justify-center">
                  <span
                    class="px-1.5 py-1 rounded text-[10px] font-bold border {getXPUScoreBadgeClass(
              ...
              <td class="px-3 py-2.5">
                <span
                  class="px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase border {getHardwareBadgeClass(
                    pr.hardware,
                  )}"
                >
                  {pr.hardware}
              ...
              <td class="px-3 py-2.5">
                <span
                  class="px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase border {getStateBadgeClass(
                    pr.state,
                  )}"
                >
                  {pr.state}
              ...
              <td class="px-3 py-2.5">
                <div class="flex flex-wrap gap-1">
                  {#each pr.types as type}
                    <span
                      class="px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase border {getTypeBadgeClass(
                        type,
                      )}"
                    >
                      {type}
                        ...
                        >
                          ...
                          <div class="space-y-2">
                            <div class="flex items-center gap-2">
                              ...
                              <span
                                class="px-2 py-1 rounded font-bold border {getXPUScoreBadgeClass(
                                  pr.xpu_relevance_score,
                                )}"
                              >
                                {pr.xpu_relevance_score}
                            ...
                            {#if pr.xpu_relevance_justification}
                              <div>
                                <span class="text-slate-600 block mb-1"
                                  >Justification:</span
                                >
                                <div
                                  class="text-slate-700 bg-slate-50 rounded p-2 text-[11px] leading-relaxed"
                                >
                        ...
                        >
                          ...
                          {#if pr.tags && pr.tags.length > 0}
                            <div class="flex flex-wrap gap-1.5">
                              {#each pr.tags as tag}
                                <span
                                  class="px-2 py-0.5 rounded text-[10px] bg-slate-100 text-slate-700 border border-slate-200"
                                >
                                  {tag}
                                </span>
                              {/each}
                      ...
                      <div class="space-y-3">
                        ...
                        {#if pr.body}
                          ...
                          >
                            ...
                            <div
                              class="text-slate-700 text-[11px] leading-relaxed max-h-40 overflow-y-auto bg-slate-50 rounded p-2"
                            >
                              {pr.body}
                            </div>
                          </div>
                      ...
                      <div class="mt-4">
                        ...
                        >
                          <div class="flex items-center gap-1.5 mb-3">
                            ...
                            <h4 class="font-semibold text-slate-800">
                              Triton Kernel Ops ({pr.triton_ops.length})
                            </h4>
                          </div>
                          <div class="space-y-2">
                            {#each pr.triton_ops as op}
                              ...
                              >
                                <button
                                  on:click={() =>
                                    toggleOpDetails(pr.number, op.op_name)}
                                ...
                                >
                                  ...
                                  >
                                    <div class="flex flex-col gap-1">
                                      <div class="flex items-center gap-2">
                                        <span
                                          class="font-mono font-bold text-slate-800 text-xs"
                                          >{op.op_name}</span
                                        >
                                        <span
                                          class="px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase border {getStatusBadgeClass(
                                            op.meta.status,
                                          )}"
                                        >
                                          {op.meta.status}
                                      ...
                                      >
                                        <span
                                          class="text-[10px] text-slate-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200"
                                        >
                                  ...
                                  <div class="flex items-center gap-3">
                                    <div class="flex flex-col items-end gap-1">
                                      <div class="flex items-center gap-1">
                                        ...
                                        >
                                        <span
                                          class="text-[9px] font-semibold {op.torch_inductor_triton.toLowerCase() ===
                                          'supported'
                                            ? 'text-green-600'
                                            : 'text-slate-600'}"
                                        >
                                      ...
                                      <div class="flex items-center gap-1">
                                        <span class="text-[9px] text-slate-500"
                                          >Official Triton:</span
                                        >
                                        <span
                                          class="text-[9px] font-semibold text-blue-600"
                                        >
                                          {op.official_triton.summary
                                            ? "Available"
                                            : "N/A"}
                                        </span>
                                      </div>
                                ...
                                {#if isOpExpanded(pr.number, op.op_name)}
                                  <div
                                    class="p-3 bg-white border-t border-slate-200"
                                  >
                                    <div class="space-y-3 text-[11px]">
                                      <div>
                                        <span
                                          class="font-semibold text-slate-700 block mb-1"
                                          >Description:</span
                                        >
                                        <p
                                      ...
                                      >
                                        <div
                                          class="bg-slate-50 rounded p-2 border border-slate-200"
                                        >
                                          <span
                                            class="font-semibold text-slate-700 block mb-1.5"
                                            >TorchInductor Triton:</span
                                          >
                                          <span
                                            class="font-mono px-2 py-1 rounded text-[10px] {op.torch_inductor_triton.toLowerCase() ===
                                            'supported'
                                              ? 'text-green-700 bg-green-50 border border-green-200'
                                              : 'text-slate-700 bg-slate-100 border border-slate-200'} inline-block"
                                          >
                                            {op.torch_inductor_triton}
                                          </span>
                                        </div>
                                        ...
                                        >
                                          ...
                                          {#if op.official_triton.summary}
                                            <p
                                              class="text-slate-600 text-[10px] leading-relaxed"
                                            >
                                              {op.official_triton.summary}
                                            </p>
                                          {:else}
                                      ...
                                      <div
                                        class="bg-blue-50 rounded p-2 border border-blue-200"
                                      >
                                        <span
                                          class="font-semibold text-slate-700 block mb-1.5"
                                          >Backend Hint:</span
                                        ...
                                        <span class="text-slate-600"
                                          >{op.backend_hint}</span
                                        >
                                        <span
                                          class="font-semibold text-slate-700 block mt-2 mb-1"
                                          >Pattern:</span
                                        >
                                        <span
                                          class="font-mono text-blue-700 text-[10px] bg-white px-1.5 py-0.5 rounded"
                                          >{op.likely_triton_pattern}</span
                                        >
                                      </div>
                                      ...
                                      >
                                        ...
                                        >
                                        <code
                                          class="text-[10px] text-slate-600 bg-white px-2 py-1 rounded block overflow-x-auto"
                                          >{op.meta.func_signature}</code
                                        >
                                      </div>
                                      ...
                                      <div class="grid grid-cols-2 gap-2">
                                        <div>
                                          <span
                                            class="font-semibold text-slate-600 block mb-1"
                                            >CUDA Function:</span
                                          >
                                          <span
                                            class="font-mono text-[10px] {op
                                              .meta.cuda_func
                                              ? 'text-green-700'
                                              : 'text-slate-400'}"
                                          >
                                        ...
                                        <div>
                                          ...
                                          <div class="flex flex-wrap gap-1.5">
                                            {#each Object.entries(op.meta.dispatches) as [backend, func]}
                                              <span
                                                class="px-1.5 py-0.5 rounded text-[9px] bg-slate-100 text-slate-700 border border-slate-200"
                                              >
                                      ...
                                      <div
                                        class="text-[10px] text-slate-500 pt-2 border-t border-slate-200"
                                      >
                                        <span class="font-semibold"
                                          >Source:</span
                                        >
  ...
  {#if totalPages > 1}
    <div
      class="px-4 py-3 border-t border-slate-200 bg-slate-50/50 flex items-center justify-between"
    >
      <div class="text-xs text-slate-600">
        Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(
          currentPage * itemsPerPage,
          allFilteredPRs.length,
        )} of {allFilteredPRs.length}
      </div>
          ...
          {#each pageNumbers as pageNum}
            ...
            {:else}
              <button
                ...
                class="px-2.5 py-1 rounded text-xs font-medium transition {pageNum ===
                currentPage
